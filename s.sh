...
sleep 2
	...
	clear
	echo -e $G"[*]$RE Restaurando termux.."
	sleep 2
...
clear
while [ "$choice" != "&" ]; do
choice=$(whiptail --title "MILUX $version" --menu "Hola $name, Selecciona que quieres instalar." 19 60 11 \
"1" "Módulo YouTube " \
"2" "SQLMAP" \
"3" "Websploit" \
...
10)
	clear
	wget https://raw.githubusercontent.com/miluxmil/milux/master/payloadgen.sh
	mv payloadgen.sh ~/.milux/payloadgen.sh
	echo "bash ~/.milux/payloadgen.sh" > infogen
	chmod +x infogen
	mv infogen $PREFIX/bin/infogen
	clear
	echo -e $G"[\033[0m*$G]$Y Instalado exitosamente ejecutalo escribiendo infogen (Enter para continuar) $RE"
	read enter
0)
clear
...
if [ "$nvers" = "$cvers" ]; then
	clear
	rm cver nver milux.sh
	echo -e $G"[\033[0m*$G]$Y Tienes la version mas reciente! $RE"
	sleep 3
	
else
	clear
	rm cver nver
	echo -e $G"[\033[0m*$G]$Y Hay una version nueva disponoble! instalar? s/n$RE"
	read -p ">> " sino
	if [ $sino = s ]; then
		mv milux.sh ~/.milux/milux.sh
		echo "bash ~/.milux/milux.sh" > milux
		chmod +x milux;mv milux $PREFIX/bin/
		echo -e $G"[\033[0m*$G]$Y Ahora puedes ejecutarlo solo escribiendo milux $RE
		"
		echo -e $G"[\033[0m*$G]$RE ENTER Para continuar."
	read enter
else
	rm cver nver milux.sh
	echo -e $G"[\033[0m*$G]$Y No estas actualizado! $RE"
	sleep 1
	echo -e $G"[\033[0m*$G]$Y Regreasando al menu.. $RE"
	sleep 4
fi
